...
class Analysis: 
    def __init__(self, config: Dict[str, Any], overhead: float=1.8):
        self.config= config
        
        self.file= self.config.data.input_path
        self.null_th= self.config.eda.thresholds.null_threshold
        
        self.frame= self.get_frame(overhead=overhead)
    
    def get_frame(self, overhead: float=1.8) -> pl.DataFrame: 
        memory_available= psutil.virtual_memory().available
        estimated_memory= self.file.stat().st_size * overhead
        
        ratio= estimated_memory/memory_available
        
        if ratio > 0.65: 
            logger.error(f'The file {self.file.name} is too large to be process. Ratio: {ratio:.2f}')
            raise ValueError(f'The file {self.file.name} is too large to be process. Ratio: {ratio:.2f}')
        else: 
            logger.info(f'The frame for the file {self.file.name} was obtained succesfully')
            return pl.read_csv(self.file)
